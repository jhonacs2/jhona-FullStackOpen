...
const Menu = ({ anecdotes, addNew, anecdoteId, setNotification }) => {
  const padding = {
    paddingRight: 5,
  };
  return (
    <div>
      ...
      <Switch>
        <Route path='/anecdotes/:id'>
          <ViewAnecdote anecdote={anecdoteId} />
        </Route>
        <Route path='/create'>
...
const CreateNew = ({ addNew, setNotification }) => {
  ...
  const [infor, resetInfor] = useField('text');

  const mainmenu = useHistory();
...
const NotificationAnecdote = ({ content }) => {
  if (content === '') {
    return null;
  }
  return (
    <div>
      <p>Anenecdote {content} added</p>
    </div>
  );
};
...
const App = () => {
  ...
  return (
    <div>
      ...
      <NotificationAnecdote content={notification} />
      <Menu
        anecdotes={anecdotes}
        addNew={addNew}
